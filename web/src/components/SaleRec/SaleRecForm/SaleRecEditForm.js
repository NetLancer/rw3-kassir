  ...
  return (
    <div className="rw-form-wrapper">
      <Form onSubmit={onSubmit}>
        ...
        <OfficesQryCell activeOnly={false} ifOfficeId={saleRec.officeId} onOfficeSelect={e => {setOfficeId(e.target.value)}} /> 
        
        <LabelGen name="employeeId">Employee Id</LabelGen>   
        <EmployeesQryCell activeOnly={false} ifEmployeeId={saleRec.employeeId} officeId={officeId} />
        

        <LabelGen name="consId">ConsolidatorId</LabelGen> 
        <ConsolidatorsQryCell activeOnly={false} ifConsId={saleRec.consId} onConsSelect={c => {setConsId(c.target.value)}} />


        <LabelGen name="carrierId">Carrier Id</LabelGen> 
        <CarriersQryCell activeOnly={false} ifCarrierId={saleRec.carrierId} consId={consId} />
        ...
        <div className="rw-button-group"> 
          <b className="border rounded-lg py-4 px-2 mr-8">
            <Link to={routes.saleRec({ id: saleRec.id })} className="text-indigo-600"> 
              Cancel editing
            </Link>
          </b> 
          {' '}
